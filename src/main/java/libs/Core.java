...
public class Core {
    ...
    private HashMap<String, LdapSearchThread> ldapSearchStack = null;
    private HashMap<String, Companys> ldapSaveCompanys = null;
    //private ArrayList<LdapSearchThread> ldapSearchStack = null;
    //private LdapSearchThread[] ldapSearchStack = null;
     ...
     */
    private LdapSearchThread ldapSearch(AbstractConnectDescriber connectDescriber) {
        LdapSearchThread ldapSearch = new LdapSearchThread(this, connectDescriber);
        ldapSearch.execute();

        return ldapSearch;
    }

    private void ldapSearchStackTread() {
        if (null == ldapSearchStack) {
            ...
            while (getSystemEnv().hasNext()) {
                ...
                if (null == ldapSearchStack.get((Object) key)) {
                    ldapSearchStack.put(key, ldapSearch(connectDescriber));
                } else {
                    LdapSearchThread ldapSearch = ldapSearchStack.get((Object) key);
                    if (ldapSearch.isDone()) {
                        ldapSearchStack.put(key, ldapSearch(connectDescriber));
                    } else {
                        ldapSearch.execute();
                    }
     ...
     * @param ldapCompanys
     */
    private void isLdapSearchSuccessful(Companys ldapCompanys) {
        setCompanys(ldapCompanys)
                .dataLoad()
                .localWriteCache();
    }
     ...
     * выгрузка из ldap успешна
     */
    public Core addLdapSearchSuccessful(Companys ldapCompanys) {

        if (null == this.companys) {
            this.companys = ldapCompanys;
        } else {
            this.companys.mergeCompanys(ldapCompanys);
        }
     ...
     * выгрузка из ldap успешна
     */
    public Core addLdapSearchSuccessful(Companys ldapCompanys, String connectionName) {
        if (null == ldapSaveCompanys) {
            ldapSaveCompanys = new HashMap<String, Companys>();
        }
        ldapSaveCompanys.put(connectionName, ldapCompanys);

        return this;
    }

    public Core removeLdapSearchStack(String connectionName) {
        ldapSearchStack.remove(connectionName);

        return this;
    }

    public Core linkCompanys() {
        if (ldapSearchStack.size() == 0) {
            getSystemEnv().reset();
            while (getSystemEnv().hasNext()) {
                AbstractConnectDescriber connectDescriber = getSystemEnv().getNext();
